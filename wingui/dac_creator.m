...
set(handles.h_edit,'String',num2str(0));
...
% h - cutting height
h=str2num(get(handles.h_edit,'String'))/1000;
...
HEIGHT{achop_indices(ind),1}=num2str(h);
MODE_2{achop_indices(ind),1}='D';
SPEED{achop_indices(ind),1}=int2str(v_schnitt);
...
INDEX{idx_y_indices(ind),1}=num2str((-1)^ind*s*ind/1000);
...
for ind=1:30;
% height values are saved in [mm] but are printed in [nm] -> *1000000
fprintf(fileID,formatSpec22,num2str(str2double(PROGRAM{ind,5})*1000000));
end;
...
for ind=1:30;
% cutting speed values are saved in [mm/s] but are printed in [nm/s] -> *1000000
fprintf(fileID,formatSpec22,num2str(str2double(PROGRAM{ind,7})*1000000));
end;
fprintf(fileID,formatSpec23);
...
for ind=1:30;
% offset values are saved in [mm] but are printed in [nm] -> *1000000
fprintf(fileID,formatSpec22,num2str(str2double(PROGRAM{ind,8})*1000000));
end;
fprintf(fileID,formatSpec23);
